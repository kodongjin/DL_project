...
+ 김도영 (kimdoyoung1023@seoultech.ac.kr)
  + 프로젝트 관리
    + 딥러닝 프로젝트 및 음성 데이터 처리 경험을 활용하여, 전반적인 프로젝트의 방향성과 목표를 구상하였습니다.
    + 팀원들의 관심사와 역량에 맞는 업무를 배정하여, 프로젝트 목표를 성공적으로 달성하기 위해 노력했습니다.
  + 모델 학습
    + 모델 학습과 관련된 모든 과정을 담당하여 수행하였습니다.
    + 서버에 데이터셋을 확보하여, 전처리 및 합성 과정을 수행하였습니다.
    + GPU를 활용하여 총 10가지 서로 다른 실험을 시행하였고, 결과를 정리하였습니다.
  + insight
    + 다양한 모델 구조와 학습 기법을 적용하면서, 몇몇 case에서 실제 예상했던 방향과 다른 결과를 확인할 수 있었습니다.
    + 이에 대해, 원인을 분석하고 또 insight를 얻을 수 있었습니다.
    
+ 곽동욱 (charles2659@naver.com)
  + 데이터 수집
    + 사용할 배경소음과 이벤트소음의 클래스를 정하고, 해당 데이터 샘플들을 수집하였습니다.
  + 웹 구현
    + 파이썬의 streamlit 도구를 활용하여 우리의 모델이 실제로 어떻게 작동하고 얼마나 정확하게 할 수 있는지를 보이기 위해 웹에서 구현하였습니다.
# 0. 데이터셋 다운로드
